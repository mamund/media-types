...
When applied to HTTP, any +model+ value associated with a +data+ element with the +action+ property set to +read+ or +remove+ MUST be converted into a valid query string. The following example shows how an UBER message snippet is converted into a valid HTTP query string:
...
Any +model+ value associated with a +data+ element with the +action+ property set to +append+, +partial+, or +replace+ MUST be convereted into a valid HTTP request body. The following example shows how an UBER message snippet is converted into a valid HTTP request body:
