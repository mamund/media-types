...
There are no Uber-specific link relation values. Document authors are SHOULD use registered link relation values that comply with those described in RFC5988 <<rfc5988, [RFC5988]>> and from shared public sources such as:
...
== Acknowledgements
The author would like to thank everyone who commented upon, encouraged, and gave feedback to this specification, especially Matthew McClure.
